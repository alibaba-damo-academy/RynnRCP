...
from lerobot_so101.utils import enter_pressed, move_cursor_up
...
class MotorsBus(abc.ABC):
    ...
    ```bash
    Use the SO101 configuration helper to find the serial port.
    ...
    def _connect(self, handshake: bool = True) -> None:
        try:
            ...
            raise ConnectionError(
                ...
                "\nCheck the configured SO101 serial port.\n"
            ) from e
    ...
    def scan_port(cls, port: str, *args, **kwargs) -> dict[int, list[int]]:
        ...
        for baudrate in bus.available_baudrates:
            ...
            if ids_models:
                print(f"Motors found for {baudrate=}: {pformat(ids_models, indent=4)}")
