...
## 概述
本文件夹内包含了 `xtrainer_servers` 和 `motion_node` 两个核心组件，`xtrainer_servers` 负责与云端及设备进行通信和管理， `motion_node` 则处理和机器人本体的数据交互。
...
```yaml
cameras:
- name: observation.images.cam_main      # 头部摄像头的名称
  device: 0                              # 摄像头的序号
...
- name: observation.images.cam_left_arm  # 左臂摄像头的名称
  device: 1                              # 摄像头的序号
  brand: Brand Name                      # 摄像头的品牌名称
  format: YUYV                           # 视频格式，例如 YUYV
  width: 640                             # 捕获图像的宽度
  height: 480                            # 捕获图像的高度
  framerate: 30                          # 每秒捕获的帧数
- name: observation.images.cam_right_arm # 右臂摄像头的名称
  device: 2                              # 摄像头的序号
  brand: Brand Name                      # 摄像头的品牌名称
  format: YUYV                           # 视频格式，例如 YUYV
  width: 640                             # 捕获图像的宽度
...
服务端代码位于 `xtrainer_servers` 目录，主要功能包括：
- 日志设置：通过配置文件初始化日志系统
...
### 构建服务节点
确保已经执行过一键安装脚本 `deploy_linux.sh` 或者 `deploy_mac.sh` ，该脚本会编译本目录文件并生成可执行程序 `xtrainer_servers` 。
...
- 控制节点依赖越疆 dobot_xtrainer SDK，在执行前需要保证 dobot_xtrainer SDK 路径已经在环境变量中（export PYTHONPATH=/path/to/dobot_xtrainer:$PYTHONPATH）
