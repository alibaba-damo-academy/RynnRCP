#!/usr/bin/env python3
"""
FR3 Motion Controller - 7-DOF robot arm + gripper
...
from rcp_motion.robots.franka.interface.fr3_interface import create_robot_interface
...
class FR3MotionController:
    ...
    def __init__(
        ...
        self.config_path = "configs/fr3.yaml"
    ...
    def _init_logging(self):
        ...
        log_dir = os.path.expanduser(
            self.config.get("logging", {}).get("log_dir", "~/logs/fr3")
        )
        log_file = self.config.get("logging", {}).get("log_file", "fr3_motion.log")
    ...
    def _init_robot_model(self):
        ...
        robotmodel_config = {
            ...
            "robot_mjcf": str(models_root / "franka/franka_fr3/scene/scene.xml"),
            "pino_mjcf": str(models_root / "franka/franka_fr3/urdf/fr3.xml"),
        }
    ...
    def motion_planner(self, motion: int, current_time: float) -> np.ndarray:
        ...
        # FR3 has 7 joints + gripper = 8 DOF
        if motion == 1:
            # Wave motion
            target = np.array([0.0, -np.pi/4, 0.0, -3*np.pi/4, 0.0, np.pi/2, np.pi/4, 0.0])
            if current_time <= duration:
                ...
                target[2] += amplitude * np.sin(2 * np.pi * frequency * t + np.pi/2)
                target[4] += amplitude * np.sin(2 * np.pi * frequency * t + np.pi)
                return target

        elif motion == 2:
            # Circular motion in joint space
            if current_time <= duration:
                target = np.array([0.0, 0.0, 0.0, -np.pi/2, 0.0, np.pi/2, 0.0, 0.0])
                return lerp(
                    self.initial_joint_positions.copy(), target, current_time, duration
                )
            else:
            ...
            # Reach forward
            if current_time <= duration:
                target = np.array([0.0, -np.pi/6, 0.0, -2*np.pi/3, 0.0, np.pi/2, 0.0, 0.0])
                return lerp(
                    self.initial_joint_positions.copy(), target, current_time, duration
                )
            else:
                ...
                target[5] += 0.5 * amplitude * np.sin(2 * np.pi * frequency * t)
                return target

        elif motion == 4:
            # Side-to-side
            target = np.array([np.pi/4, -np.pi/4, 0.0, -np.pi/2, 0.0, np.pi/3, 0.0, 0.0])
            if current_time <= duration:
                return lerp(
                    self.initial_joint_positions.copy(), target, current_time, duration
                )
                ...
                t = current_time - duration
                target[0] += amplitude * np.sin(2 * np.pi * frequency * t)
                target[6] += amplitude * np.sin(2 * np.pi * frequency * t)
                return target
    ...
    def run(self):
        self.logger.info(f"FR3 Motion Controller: mode={self.mode}, motion={self.motion}, freq={self.frequency}Hz")
...
def main():
    parser = argparse.ArgumentParser(description="FR3 Motion Controller - Motion Patterns (1-5)")
    ...
    controller = FR3MotionController(
        mode=args.mode,
        motion=args.motion,
