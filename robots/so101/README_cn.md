...
- 端侧启动示例：
  - run_rcp_so101.py：启动 RcpCore + RynnBot(plugin)，用于连接乐云平台/上层控制系统
- 配置向导脚本：
  - configure_so101.py：交互式配置（支持中文/英文），可自动完成相机、串口、标定及平台三元组填写以及机械臂标定
...
│   └── so101_config.yaml       # RcpCore 配置：Action/Sensor/DeviceMonitor
