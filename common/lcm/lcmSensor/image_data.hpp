/** THIS IS AN AUTOMATICALLY GENERATED FILE.  DO NOT MODIFY
 * BY HAND!!
 *
 * Generated by lcm-gen 1.5.1
 **/

#ifndef __lcmSensor_image_data_hpp__
#define __lcmSensor_image_data_hpp__

#include <lcm/lcm_coretypes.h>

#include <string>
#include <vector>

namespace lcmSensor
{

class image_data
{
    public:
        int32_t    camera_id;

        /**
         * The camera id that provided the image
         */
        std::string camera_name;

        /**
         * The name of the camera
         */
        int32_t    width;

        /**
         * Width of the image
         */
        int32_t    height;

        /**
         * Height of the image
         */
        int32_t    channels;

        /**
         * Number of channels (e.g., 3 for RGB, 1 for grayscale)
         */
        int32_t    image_size;

        /**
         * Size of the image data
         * LCM Type: byte[image_size]
         */
        std::vector< uint8_t > image_bytes;

    public:
        /**
         * Encode a message into binary form.
         *
         * @param buf The output buffer.
         * @param offset Encoding starts at thie byte offset into @p buf.
         * @param maxlen Maximum number of bytes to write.  This should generally be
         *  equal to getEncodedSize().
         * @return The number of bytes encoded, or <0 on error.
         */
        inline int encode(void *buf, int offset, int maxlen) const;

        /**
         * Check how many bytes are required to encode this message.
         */
        inline int getEncodedSize() const;

        /**
         * Decode a message from binary form into this instance.
         *
         * @param buf The buffer containing the encoded message.
         * @param offset The byte offset into @p buf where the encoded message starts.
         * @param maxlen The maximum number of bytes to read while decoding.
         * @return The number of bytes decoded, or <0 if an error occured.
         */
        inline int decode(const void *buf, int offset, int maxlen);

        /**
         * Retrieve the 64-bit fingerprint identifying the structure of the message.
         * Note that the fingerprint is the same for all instances of the same
         * message type, and is a fingerprint on the message type definition, not on
         * the message contents.
         */
        inline static int64_t getHash();

        /**
         * Returns "image_data"
         */
        inline static const char* getTypeName();

        // LCM support functions. Users should not call these
        inline int _encodeNoHash(void *buf, int offset, int maxlen) const;
        inline int _getEncodedSizeNoHash() const;
        inline int _decodeNoHash(const void *buf, int offset, int maxlen);
        inline static uint64_t _computeHash(const __lcm_hash_ptr *p);
};

int image_data::encode(void *buf, int offset, int maxlen) const
{
    int pos = 0, tlen;
    int64_t hash = getHash();

    tlen = __int64_t_encode_array(buf, offset + pos, maxlen - pos, &hash, 1);
    if(tlen < 0) return tlen; else pos += tlen;

    tlen = this->_encodeNoHash(buf, offset + pos, maxlen - pos);
    if (tlen < 0) return tlen; else pos += tlen;

    return pos;
}

int image_data::decode(const void *buf, int offset, int maxlen)
{
    int pos = 0, thislen;

    int64_t msg_hash;
    thislen = __int64_t_decode_array(buf, offset + pos, maxlen - pos, &msg_hash, 1);
    if (thislen < 0) return thislen; else pos += thislen;
    if (msg_hash != getHash()) return -1;

    thislen = this->_decodeNoHash(buf, offset + pos, maxlen - pos);
    if (thislen < 0) return thislen; else pos += thislen;

    return pos;
}

int image_data::getEncodedSize() const
{
    return 8 + _getEncodedSizeNoHash();
}

int64_t image_data::getHash()
{
    static int64_t hash = static_cast<int64_t>(_computeHash(NULL));
    return hash;
}

const char* image_data::getTypeName()
{
    return "image_data";
}

int image_data::_encodeNoHash(void *buf, int offset, int maxlen) const
{
    int pos = 0, tlen;

    tlen = __int32_t_encode_array(buf, offset + pos, maxlen - pos, &this->camera_id, 1);
    if(tlen < 0) return tlen; else pos += tlen;

    char* camera_name_cstr = const_cast<char*>(this->camera_name.c_str());
    tlen = __string_encode_array(
        buf, offset + pos, maxlen - pos, &camera_name_cstr, 1);
    if(tlen < 0) return tlen; else pos += tlen;

    tlen = __int32_t_encode_array(buf, offset + pos, maxlen - pos, &this->width, 1);
    if(tlen < 0) return tlen; else pos += tlen;

    tlen = __int32_t_encode_array(buf, offset + pos, maxlen - pos, &this->height, 1);
    if(tlen < 0) return tlen; else pos += tlen;

    tlen = __int32_t_encode_array(buf, offset + pos, maxlen - pos, &this->channels, 1);
    if(tlen < 0) return tlen; else pos += tlen;

    tlen = __int32_t_encode_array(buf, offset + pos, maxlen - pos, &this->image_size, 1);
    if(tlen < 0) return tlen; else pos += tlen;

    if(this->image_size > 0) {
        tlen = __byte_encode_array(buf, offset + pos, maxlen - pos, &this->image_bytes[0], this->image_size);
        if(tlen < 0) return tlen; else pos += tlen;
    }

    return pos;
}

int image_data::_decodeNoHash(const void *buf, int offset, int maxlen)
{
    int pos = 0, tlen;

    tlen = __int32_t_decode_array(buf, offset + pos, maxlen - pos, &this->camera_id, 1);
    if(tlen < 0) return tlen; else pos += tlen;

    int32_t __camera_name_len__;
    tlen = __int32_t_decode_array(
        buf, offset + pos, maxlen - pos, &__camera_name_len__, 1);
    if(tlen < 0) return tlen; else pos += tlen;
    if(__camera_name_len__ > maxlen - pos) return -1;
    this->camera_name.assign(
        static_cast<const char*>(buf) + offset + pos, __camera_name_len__ - 1);
    pos += __camera_name_len__;

    tlen = __int32_t_decode_array(buf, offset + pos, maxlen - pos, &this->width, 1);
    if(tlen < 0) return tlen; else pos += tlen;

    tlen = __int32_t_decode_array(buf, offset + pos, maxlen - pos, &this->height, 1);
    if(tlen < 0) return tlen; else pos += tlen;

    tlen = __int32_t_decode_array(buf, offset + pos, maxlen - pos, &this->channels, 1);
    if(tlen < 0) return tlen; else pos += tlen;

    tlen = __int32_t_decode_array(buf, offset + pos, maxlen - pos, &this->image_size, 1);
    if(tlen < 0) return tlen; else pos += tlen;

    if(this->image_size) {
        this->image_bytes.resize(this->image_size);
        tlen = __byte_decode_array(buf, offset + pos, maxlen - pos, &this->image_bytes[0], this->image_size);
        if(tlen < 0) return tlen; else pos += tlen;
    }

    return pos;
}

int image_data::_getEncodedSizeNoHash() const
{
    int enc_size = 0;
    enc_size += __int32_t_encoded_array_size(NULL, 1);
    enc_size += this->camera_name.size() + 4 + 1;
    enc_size += __int32_t_encoded_array_size(NULL, 1);
    enc_size += __int32_t_encoded_array_size(NULL, 1);
    enc_size += __int32_t_encoded_array_size(NULL, 1);
    enc_size += __int32_t_encoded_array_size(NULL, 1);
    enc_size += __byte_encoded_array_size(NULL, this->image_size);
    return enc_size;
}

uint64_t image_data::_computeHash(const __lcm_hash_ptr *)
{
    uint64_t hash = 0xa89519035db244afLL;
    return (hash<<1) + ((hash>>63)&1);
}

}

#endif
