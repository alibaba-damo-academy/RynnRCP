#!/usr/bin/env bash
# Build the aero_hand_mtnn pybind module (MTNN zero-copy pipeline for the
# Moore Threads E300 / M1000 NPU).
#
...
# Requires the system MTNN runtime (libmtnnrt.so) and NPU headers only.
set -euo pipefail
...
echo "built: $(ls "$HERE/../rynnrcp_robot_aero_hand/accelerators/"aero_hand_mtnn*.so)"
