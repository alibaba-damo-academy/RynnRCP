#!/usr/bin/env python3
"""
SO100 Robot Configuration Tool (Bilingual, minimal-field changes)

Added:
- Camera visual mapper (DearPyGui) preferred; fallback to hotplug on CLI/unavailable GUI.
- Writes mapping directly into robots/so100/config/so100_config.yaml (NOT temp json).

Only updates in so100_config.yaml:
...
SO100_RCP_CONFIG_PATH = SCRIPT_DIR / "config" / "so100_config.yaml"
SO100_LOWLEVEL_CONFIG_PATH = (
...
SO100_ROBOT_DIR = SCRIPT_DIR.parent.parent / "rcp_motion" / "robots" / "so100"
...
I18N: Dict[str, Dict[str, str]] = {
    ...
    "tool_title": {"zh": "SO100 配置工具", "en": "SO100 CONFIGURATION TOOL"},
    "welcome": {
        "zh": "欢迎使用 SO100 配置工具！请按提示完成配置。\n日志会输出到控制台，便于排查问题。\n",
        "en": "Welcome! Follow the prompts to configure SO100.\nLogs are printed to console for troubleshooting.\n",
    },
...
def configure_cameras_gui(max_probe: int = 8) -> Optional[Dict[str, str]]:
    ...
    with dpg.window(label="SO100 Camera Mapper", width=1200, height=780):
        with dpg.group(horizontal=True):
...
def configure_cameras() -> None:
    ...
    if mapping:
        ...
        save_yaml_config(SO100_RCP_CONFIG_PATH, cfg)
...
def calibrate_robot_arm() -> None:
    ...
    try:
        ...
        if not SO100_ROBOT_DIR.exists():
            raise SystemExit(t("dir_missing", path=str(SO100_ROBOT_DIR)))

        result = subprocess.run(
            [sys.executable, "-m", "scripts.follower_calibrate", "--mode", "manual"],
            cwd=str(SO100_ROBOT_DIR),
        )
