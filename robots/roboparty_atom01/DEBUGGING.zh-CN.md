...
setup 会安装 RynnBot、MCP 和 Protocol Debug App。Server 启动后按需另开终端：
...
推荐在 Atom01 机器人本机运行安装脚本。脚本会创建 `robots/roboparty_atom01/venv`，安装
...
```bash
bash setup_atom01.sh --skip-apt --recreate
```
...
日常安装使用 `setup_atom01.sh`。需要手动排查构建过程时，可按下面命令逐步执行。

```bash
python -m pip install -e .
python -m pip install -e apps/common -e apps/rynnbot
python -m pip install -e robots/roboparty_atom01
```
