...
class Policy:
    def load(self, policy_dir):
        ...
        self.frame_stack = 3
        self.obs_layout = [("ang_vel", 3), ("gravity_b", 3), ("cmd_vel", 3), ("dof_pos", 23), ("dof_vel", 23), ("last_action", 23)]
    ...
    def step(self, obs):
        ...
        cmd_vel = np.array([
            np.clip(raw_cmd_vel[0], -0.5, 2.0),
            np.clip(raw_cmd_vel[1], -0.6, 0.6),
            np.clip(raw_cmd_vel[2], -1.57, 1.57),
        ], dtype=np.float32)
        segments = [
    ...
    def _push_obs_major(self, segments):
        segments = [np.clip(segment, -100.0, 100.0).astype(np.float32) for segment in segments]
        offset = 0
        for segment in segments:
            size = len(segment)
            end = offset + size * self.frame_stack
            if self.first:
                for frame in range(self.frame_stack):
                    self.input_buffer[offset + frame * size:offset + (frame + 1) * size] = segment
            else:
                self.input_buffer[offset:end - size] = self.input_buffer[offset + size:end]
                self.input_buffer[end - size:end] = segment
            offset = end
        self.first = False
