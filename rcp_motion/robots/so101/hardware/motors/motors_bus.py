...
from rcp_motion.robots.so101.hardware.utils.utils import enter_pressed, move_cursor_up
...
class MotorsBus(abc.ABC):
    ...
    def _compute_normalized_display(self, motor: str, raw_pos: int, range_min: int, range_max: int) -> str:
        """Compute normalized value for display during calibration.

        Args:
            motor: Motor name
            raw_pos: Raw encoder position
            range_min: Minimum encoder value from calibration
            range_max: Maximum encoder value from calibration

        Returns:
            Formatted string showing normalized value (radians for joints, 0-1 for gripper)
        """
        if range_max == range_min:
            return "N/A"

        norm_mode = self.motors[motor].norm_mode
        bounded_val = min(range_max, max(range_min, raw_pos))

        if norm_mode is MotorNormMode.RANGE_0_1:
            # Gripper: show 0-1 range
            norm = (bounded_val - range_min) / (range_max - range_min)
            return f"{norm:.3f}"
            ...
            model = self.motors[motor].model
            max_res = self.model_resolution_table[model] - 1
            degrees = (raw_pos - mid) * 360 / max_res
            radians = math.radians(degrees)
            return f"{radians:+.3f}"
        elif norm_mode is MotorNormMode.RANGE_M100_100:
            # Convert -100 to 100 range to radians (assume ±180 degrees = ±π)
            import math
            norm = (((bounded_val - range_min) / (range_max - range_min)) * 200) - 100
            # Map -100..100 to -π..π
            radians = norm * math.pi / 100
            return f"{radians:+.3f}"
        elif norm_mode is MotorNormMode.RANGE_0_100:
            # Convert 0-100 range to radians (assume 0-180 degrees = 0-π)
            import math
    ...
    def record_ranges_of_motion(
        ...
        while not user_pressed_enter:
            ...
            if display_values:
                print("\n" + "-" * 70)
                print(f"{'NAME':<15} | {'MIN':>6} | {'POS':>6} | {'MAX':>6} | {'NORMALIZED':>10}")
                for motor in motors:
                    # Compute normalized value using current min/max as calibration range
                    norm_str = self._compute_normalized_display(
                        motor, positions[motor], mins[motor], maxes[motor]
                    )
                    print(f"{motor:<15} | {mins[motor]:>6} | {positions[motor]:>6} | {maxes[motor]:>6} | {norm_str:>10}")
