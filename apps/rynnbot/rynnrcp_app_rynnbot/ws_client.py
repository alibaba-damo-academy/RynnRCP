# rynnrcp_app_rynnbot/ws_client.py

"""
WebSocket transport client for the Rynnbot app.
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

This module provides :class:`~rynnrcp_app_rynnbot.ws_client.RynnWebsocketClient`,
a wrapper around ``websocket-client`` that connects to the Rynnbot WebSocket gateway
using an access token obtained from :class:`~rynnrcp_app_rynnbot.auth.RynnAuthClient`.
...
class RynnWebsocketClient:
    ...
    def _on_message(self, ws, message) -> None:
        ...
        if isinstance(message, bytes):
            payload = message
            logger.info("[WS] Binary message received, size=%d", len(message))
    ...
    def _ws_run_forever(self) -> None:
        ...
        while self._running.is_set():
            try:
                ...
                logger.info("[WS] Connecting to %s ...", self._build_url())
                self._ws.run_forever(sslopt={"cert_reqs": ssl.CERT_NONE})
            except Exception as e:
                logger.error("[WS] run_forever error: %s", e, exc_info=True)
    ...
    def send_bytes(self, data: bytes) -> None:
        ...
        try:
            ...
            logger.warning("[WS] send_bytes error: %s", e, exc_info=True)
