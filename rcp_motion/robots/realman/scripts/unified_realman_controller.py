...
from interface.rm75_interface import (
    create_rm75_interface,
)
...
class RealmanController:
        ...
        if self.signal_source == "sim":
            ...
            self.signal_handler.set_traj_parameter(
                home_position=signal_params.get(
                    "signal_home_position", [0, 0, 0, -0.0, 0, 0, -0.0]
                ),
    ...
    def init_interface(self):
        ...
        try:
            if self.mode == "real":
                self.robot_interface = create_rm75_interface(
    ...
    def _process_lcm_trajectory(self, new_chunk):
        ...
        for i in range(self.signal_chunk_size):
            ...
            qACT.extend(new_chunk.jointPos[joint_start:joint_end])

            gripper_start = i * new_chunk.numGripper
            gripper_end = gripper_start + new_chunk.numGripper
            qACT.extend(new_chunk.gripperPos[gripper_start:gripper_end])

        total_dof = new_chunk.numJoint + new_chunk.numGripper
        self.signal_trajectory = np.array(qACT[:total_dof])
    ...
    def _lcm_publishing(self):
        ...
        if self.lcm_handler.check_robot_feedback_request():
            self.lcm_handler.publish_robot_feedback(self.robot_joint_fb)
...
def main():
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="configs/rm75_config.yaml",
        help="Path to configuration file",
    )

    args = parser.parse_args()

    franka_ctrl = RealmanController(
        mode=args.mode,
