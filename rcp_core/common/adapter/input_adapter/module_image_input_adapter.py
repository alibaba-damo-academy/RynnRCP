# rcp_core/common/adapter/module_image_input_adapter.py

"""
Image input adapter for Python camera modules.
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

This module defines :class:`~rcp_core.common.adapter.module_image_input_adapter.ModuleImageInputAdapter`,
an input adapter that normalizes frames returned by Python camera modules into the
same structured dict format used by the LCM image adapters.
...
class ModuleImageInputAdapter(BaseInputAdapter):
    """
    Module image input adapter:
    ...
    def parse(self, msg: Any) -> Tuple[float, Dict[str, Any]]:
        ...
        if not isinstance(msg, (tuple, list)) or len(msg) != 5:
            raise TypeError(
                f"[ModuleImageInputAdapter] Expected msg to be (ok, width, height, encoding, data), got: {type(msg)}"
            )
        ...
        if encoding in ("rgb8", "bgr8"):
            ...
            if not isinstance(frame, (bytes, bytearray)):
                raise TypeError(
                    "[ModuleImageInputAdapter] Frame must be bytes for compressed image."
                )
            img_dict["data"] = bytes(frame)  # Ensure frame is in bytes
        else:
            raise ValueError(
                f"[ModuleImageInputAdapter] Unsupported encoding: {encoding}"
            )
