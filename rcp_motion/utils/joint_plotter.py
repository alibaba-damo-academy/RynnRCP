...
class JointPlotter:
    ...
    def __init__(
        ...
        self.velocity_data = np.zeros((self.num_joints, max_points), dtype=np.float64)
        self.current_idx = 0
        self.data_count = 0
    ...
    def generate_plots(self, save_path: Optional[str] = None, show: bool = True):
        ...
        try:
            ...
            if self.data_count > self.max_points:
                ...
                velocity_data = self.velocity_data[:, :valid_count]
            ...
            for i in range(min(self.num_joints, 6)):  # Limit to 6 joints for 2x3 layout
                ...
                ax.plot(time_array, fb_data[i], "r--", linewidth=2, label="Feedback")
            ...
            plt.tight_layout()
            if save_path:
                plt.savefig(f"{save_path}_positions.png", dpi=300, bbox_inches="tight")
                self.logger.info(f"Position plot saved to: {save_path}_positions.png")
            if show:
                plt.show()

            # Plot joint velocities
            fig2, axes2 = plt.subplots(2, 3, figsize=(15, 10))
            fig2.suptitle("Joint Velocities", fontsize=16)
            axes2 = axes2.flatten()
            for i in range(min(self.num_joints, 6)):
                ax = axes2[i]
                joint_name = (
                    self.joint_names[i] if i < len(self.joint_names) else f"Joint_{i+1}"
                )
                ax.plot(time_array, velocity_data[i], "g-.", linewidth=2, label="Velocity")
                ax.set_title(f"Joint {i+1}: {joint_name}")
                ax.set_xlabel("Time (s)")
                ...
                ax.grid(True, alpha=0.3)
                if valid_count > 0:
                    data_min = np.min(velocity_data[i])
                    data_max = np.max(velocity_data[i])
                    data_range = data_max - data_min
                    padding = max(0.1, data_range * 0.1)
                    ax.set_ylim(data_min - padding, data_max + padding)
            for i in range(self.num_joints, 6):
            ...
            plt.tight_layout()
            if save_path:
                plt.savefig(f"{save_path}_velocities.png", dpi=300, bbox_inches="tight")
                self.logger.info(f"Velocity plot saved to: {save_path}_velocities.png")
            if show:
                plt.show()
    ...
    def add_data(
        ...
        # Calculate velocity
        if self.data_count > 0:
            dt = current_time - self.time_data[self.current_idx - 1]
            if dt > 0:
                self.velocity_data[:, self.current_idx] = (command_positions - self.command_data[:, self.current_idx - 1]) / dt
