"""MediaPipe Lite cascade with RKNN (NPU) inference: backend ``mediapipe_lite_rknn``.

Only the two model inferences (palm_detection / hand_landmark .rknn,
full fp16 by default) run on the NPU. Everything else replicates the
official MediaPipe ``hand_landmark_tracking`` graph in Python:

  frame RGB -> letterbox 192 -> palm (NPU) -> SSD decode + weighted NMS
            -> rotated square ROI (wrist->middle, x2.6, shift -0.5)
            -> affine crop 224 -> hand (NPU) -> landmark projection back
  tracking  -> next-frame ROI from previous landmarks (x2.0, shift -0.1);
               palm reruns only when tracked hands < num_hands
               (presence < 0.5 drops a hand, like the official graph).

RKNN models have the /255 norm baked in and take uint8 RGB directly.

Both entry points are fp16; the int8 variants were dropped after the
2026-07-28 model-matrix evaluation:

- ``mediapipe_lite_rknn``      -> palm_detection_full_fp16.rknn +
                                  hand_landmark_full_fp16.rknn (default)
- ``mediapipe_lite_rknn_fp16`` -> palm_detection_lite_fp16.rknn +
                                  hand_landmark_lite_fp16.rknn

Models must be converted with rknn-toolkit2 using the same
``mean_values=[[0,0,0]] / std_values=[[255,255,255]]`` config (norm baked
in) and quantization disabled (``do_quantization=False``).

NOTE: models are container v6 and need librknnrt >= 2.3.x. When the system
runtime is older, setup_aero_hand.sh fetches a 2.3 runtime into
``<repo>/models/rknn_runtime/librknnrt.so`` and this backend preloads it
(RTLD_GLOBAL) before importing rknnlite, so the system copy is never
replaced.
"""
...
# Weights live in the repo-root models/ tree (not git-tracked; fetched from
# the gitee model zoo by model_store on first use). File names carry the
# target RKNN runtime version.
# The AERO_HAND_RKNN_* environment variables can override for experiments.
...
PALM_RKNN_FP16 = os.environ.get(
    "AERO_HAND_RKNN_PALM_FP16",
    os.path.join(_MODEL_DIR, "palm_detection_lite_fp16_rknn2.3.0.rknn"),
)
HAND_RKNN = os.environ.get(
    "AERO_HAND_RKNN_HAND",
    os.path.join(_MODEL_DIR, "hand_landmark_full_fp16_rknn2.3.0.rknn"),
)
HAND_RKNN_FP16 = os.environ.get(
    "AERO_HAND_RKNN_HAND_FP16",
    os.path.join(_MODEL_DIR, "hand_landmark_lite_fp16_rknn2.3.0.rknn"),
)
...
# 12 stable palm/knuckle landmarks only (fingertips excluded), so finger
# flexion does not move the crop.
...
def _preload_rknn_runtime() -> None:
    """Prefer the project-local librknnrt.so (>= 2.3) over the system copy.

    rknnlite dlopens ``librknnrt.so`` by name; loading the project copy
    first with RTLD_GLOBAL makes that dlopen bind to it, so an old system
    runtime (< 2.3, cannot load container-v6 models) is never used and the
    system path stays untouched.
    """
    path = os.path.join(model_store.models_root(), "rknn_runtime", "librknnrt.so")
...
class MediaPipeLiteRknnBackend:
    """MediaPipe Lite pipeline with palm/hand inference on the RK NPU."""

    name = "mediapipe_lite_rknn"
    ...
    def __init__(self, *, mediapipe: Any = None, max_num_hands: int = 1,
        ...
        _preload_rknn_runtime()
        from rknnlite.api import RKNNLite

        if name is not None:
            self.name = name
        palm_path = palm_model or PALM_RKNN
        hand_path = hand_model or HAND_RKNN
        model_store.ensure_backend_files((palm_path, hand_path))
        for path in (palm_path, hand_path):
            if not os.path.isfile(path):
                raise FileNotFoundError(f"rknn model not found: {path}")
        ...
        self._palm = RKNNLite()
        self._hand = RKNNLite()
        for runtime, path in ((self._palm, palm_path), (self._hand, hand_path)):
            if runtime.load_rknn(path) != 0:
                raise RuntimeError(f"failed to load rknn model: {path}")
            if runtime.init_runtime() != 0:
                raise RuntimeError(
                    f"failed to init rknn runtime for {path}; "
                    "librknnrt >= 2.3.x is required (see module docstring)"
                )
    ...
    def _detect_palms(self, image: np.ndarray) -> list[_Roi]:
        ...
        outs = self._palm.inference(inputs=[canvas[None]])
        # RKNN may emit packed layouts, e.g. (1,42,24,36)+(1,42,24,2); element
        # order matches the flat (2016,18)/(2016,) tensors (verified vs tflite).
    ...
    def _crop(self, image: np.ndarray, roi: _Roi) -> tuple[np.ndarray, np.ndarray]:
        ...
        crop = cv2.warpAffine(image, matrix, (HAND_SIZE, HAND_SIZE),
                              flags=cv2.INTER_LINEAR, borderValue=0)
        return crop, matrix

    def _roi_from_landmarks(self, points: np.ndarray) -> _Roi:
        # Official HandLandmarksToRectCalculator: rotation from wrist to the
        # weighted mean of the index/middle/ring MCP knuckles (25/50/25), and
        # the bbox over the 12 stable palm landmarks -- both damp per-point
        # noise and keep the ROI still while fingers flex.
        ...
        # project into rotated frame, take bbox there
    ...
    def process(self, image_rgb: np.ndarray) -> Any:
        ...
        for roi in rois[: self.num_hands]:
            ...
            lm, score, handed, world = self._hand.inference(inputs=[crop[None]])
    ...
    def close(self) -> None:
        for runtime in (self._palm, self._hand):
            try:
                runtime.release()
            except Exception:
                pass
...
register_gesture_backend(
    "mediapipe_lite_rknn",
    lambda **kwargs: MediaPipeLiteRknnBackend(**kwargs),
    replace=True,
)

# fp16 variant: both palm and hand run the fp16 RKNN models (norm still
# baked into the models, uint8 RGB input unchanged).
register_gesture_backend(
    "mediapipe_lite_rknn_fp16",
    lambda **kwargs: MediaPipeLiteRknnBackend(
        palm_model=PALM_RKNN_FP16, hand_model=HAND_RKNN_FP16,
        name="mediapipe_lite_rknn_fp16", **kwargs
    ),
    replace=True,
)
